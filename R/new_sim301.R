...
stats <- function(data, timepoint = seq(0, 1, 0.1)){
  ...
  res$proportion <- res$state1/res$state2
  already_censored <- function(time, dat){
    nrow(subset(dat, s1 == s2 & t2 <= time))
  }
  res$censored_number <- sapply(timepoint, already_censored, dat = data)
  res$censored_rate <- res$censored_number/length(unique(data$id))
  res
}
