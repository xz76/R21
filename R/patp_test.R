...
#######################################
sim_group <- function(data, cid){
  l <- length(unique(data$id))
  g <- rbinom(l, 1, 0.5)
  q <- quantile(l)
  rand_cid <- function(cid, l, sd = 1) {
    vec <- rnorm(cid - 1, l/(cid), sd)
    vec <-  floor(vec)
    vec[cid] <- l - sum(vec)
    vec
  }  
  cid_seq <- rand_cid(cid = cid, l = l ,sd = 1)
  ...
  for(i in 1 : length(cid_seq)){
    ...
    cid_num <- c(cid_num,tmp)
  }
  out <- do.call(rbind, by(res, res$id, function(sub_dat){
    sub_dat$cid <- rep(cid_num[sub_dat$id[1]], nrow(sub_dat))
    return(sub_dat)
  }))
  rownames(out) <- NULL
  return(out)
  ...
  if(weighted==TRUE){
    M0 <- aggregate(rep(1,times=nrow(data)),
                    by = list(data[,cid],
                              ...
                              data[,id]),
                    FUN = mean)
    M0 <- tapply(rep(1,times=nrow(data)), data[])
    M <- aggregate(M0$x,
